...
exports.mountFloatingSettingsLauncher = mountFloatingSettingsLauncher;
...
let standaloneNavRoot = null;
let standalonePanel = null;
let standaloneNavButtons = null;
let standalonePageButtons = new Map();
let standaloneKeyHandler = null;
...
function mountFloatingSettingsLauncher() {
    if (standaloneLauncher?.isConnected)
        return;
    const mount = () => {
        if (!document.body)
            return;
        const existing = document.querySelector("[data-codexpp-standalone-launcher='true']");
        if (existing) {
...
function normalizeStandalonePage(page) {
    if (!page)
        return null;
    if (page.kind === "registered" && !state.pages.has(page.id))
        return null;
    return page;
}
function refreshStandaloneNav() {
    ...
    if (pages.length > 0) {
        ...
        for (const p of pages) {
            const btn = makeStandaloneNavButton(p.page.title, p.page.iconSvg ?? defaultPageIconSvg(), () => activateStandalonePage({ kind: "registered", id: p.id }));
...
function syncStandaloneNavActive() {
    ...
    for (const [id, btn] of standalonePageButtons) {
        setStandaloneButtonActive(btn, active?.kind === "registered" && active.id === id);
    }
}
